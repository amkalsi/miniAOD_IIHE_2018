...
IIHEModuleMET::IIHEModuleMET(const edm::ParameterSet& iConfig, edm::ConsumesCollector && iC): IIHEModule(iConfig),
	metnominalWrapper_(new IIHEMETWrapper("MET_nominal"))
							     //,metWrapperEE_(new IIHEMETWrapper("MET_2018_ee"))
							     //METCollectionFilter
							     ...
							     //  metFinalWrapper_(new IIHEMETWrapper("MET_FinalCollection"))
{
	...
	pfMETTokenFilter_                         =  iC.consumes< bool >(iConfig.getParameter<edm::InputTag>("METCollectionFilter"));
...
void IIHEModuleMET::beginJob(){
	...
	if(isMC_){
		setBranchType(kBool);
		addBranch("MET_ecalBadCalibFilterUpdate_2018");
...
void IIHEModuleMET::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	...
	iEvent.getByToken(pfMETTokenFilter_,passecalBadCalibFilterUpdate);
	...
	//if(!(_passecalBadCalibFilterUpdate)) cout<<"STRAT"<<std::endl;
	store("MET_ecalBadCalibFilterUpdate_2018", _passecalBadCalibFilterUpdate);
